...
class GSPatternEvent(object):
    """
    Represents an event of a GSPattern. An event is made of start and length,
    origin pitch, velocity and associated tags.

    Attributes:
        start_time: start_time of event
    ...
    def __init__(self, start_time, duration, pitch, velocity=127, tags=[]):
    ...
    def has_one_common_tag_with(self, event):
        ...
        """
        return self.has_one_of_tags(event.tags)


    def has_one_of_tags(self, tags):
    ...
    def all_tags_are_equal_with(self, event):
    ...
    def cutInSteps(self,stepSize):
        ...
        for i in range(num):
            newE = self.copy()
            newE.startTime = self.start_time + i * stepSize
    ...
    def __repr__(self):
        return "%s %i %f %f"%(self.tags, self.pitch, self.start_time, self.duration)
...
class GSPattern(object):
    ...
    Args:
        ...
        time_signature: list of integer representing time signature ; i.e [numerator,denominator]
    """
    ...
    def getAllIdenticalEvents(self,event,allTagsMustBeEquals = True):
        ...
        for e in self.events:
            equals = False;
            equals = event.all_tags_are_equal_with(e) if allTagsMustBeEquals else event.hasOneCommonTagWith(e)
    ...
    def getPatternForTimeSlice(self,startTime,length,trimEnd = True):
        ...
        Args:
            ...
            trimEnd: cut any events that ends after start_time + length
        Returns:
    ...
    def __repr__(self):
        ...
        each line represents an event formatted as  : tags pitch start_time duration
        """
        s = "GSPattern %s\n"%(self.name)
        for e in self.events:
            s+=str(e)+"\n"
    ...
    def split_in_equal_length_patterns(self, desired_length, trim_end=True, copy=True):
        """ splits a pattern in consecutive equal length cuts

        Args:
            desired_length: length desired for each pattern
        ...
        for e in self.events:
            p = math.floor(e.startTime * 1.0 / desired_length);
            numPattern = str(p)
            if numPattern not in patterns:
                patterns[numPattern] = self.getACopyWithoutEvents()
                patterns[numPattern].name+="_slice_"+numPattern
                patterns[numPattern].duration = desired_length;
                patterns[numPattern].name = self.name + "_"+numPattern;
            newEv = e if not copy else e.copy();
            newEv.startTime-= p * desired_length;
            if trimEnd and (newEv.startTime + newEv.duration > desired_length) :
                newEv.duration = desired_length - newEv.startTime
